...
async def transcribe(video_dir: Union[Path, str]):
    ...
    if video_dir.is_dir():
        video_paths = list(video_dir.glob("*.mp3")) + list(video_dir.glob("*.mp4")) + list(video_dir.glob("*.wav"))
    else:
    ...
    transcriptions = []
    for f in tqdm(asyncio.as_completed(transcription_tasks), total=len(transcription_tasks)):
